...
public class UserController {
    ...
    public ModelAndView skipModUser(int uid, ModelAndView model) {
        ...
        model.addObject("phone",phone.replaceAll("(\\d{3})\\d{4}(\\d{4})", "$1****$2"));
        model.addObject("email",email.replaceAll("(\\w?)(\\w+)(\\w)(@\\w+\\.[a-z]+(\\.[a-z]+)?)", "$3****$5$4"));
        model.setViewName("modUser");
        return model;
    }

    @PostMapping("modUser")
    public String modUser(@RequestParam Map<String, Object> map,@RequestParam MultipartFile[] Head_portrait) throws IOException {
    ...
    @ResponseBody
    public int selUserAdmin(@RequestParam Map<String,Object> map){
        System.out.println("map:"+map);
        Map<String,Object> map1=new HashMap<>();
        map1.put("account",map.get("name"));
        map1.put("pass",map.get("password"));
        Map<String,Object> adminMap = userService.selUserAdmin(map1);
        System.out.println("adminMap:"+adminMap);
        if(adminMap==null){
            return 0;
        }else if((int)adminMap.get("Administration")==1){
            return 1;
        }else{
            return 0;
        }
    ...
    @ResponseBody
    public int setblock(@RequestParam Map<String,Object> map){
        int num=userService.setblock(map);
        return num;
    }
}
