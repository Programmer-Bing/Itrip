...
(function(_qc) {
        ...
        var reg = /\/js\/lib\/qc(?:[^/_]*)\.js(?:[?#]appId=(\d+))?/i;
        ...
        FLAProxy.getFlashHtml = function(flashArguments, requiredVersion, flashPlayerCID) {
            ...
            if (location && location.protocol.indexOf("https") < 0) {
                _attrs.push(' codeBase="https://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab#version=', requiredVersion, '"');
            }
            return "<object" + _attrs.join("") + ">" + _params.join("") + "</object>";
        }
        ;
        var _origin = "https://graph.qq.com"
            , _origin_static = "https://qzonestyle.gtimg.cn";
        ...
        _qc.api = function() {
            ...
            var bindTokenPara = function(req) {
                ...
                if (appId <= 0)
                    throw new Error("鎰忓鐨勮皟鐢ㄤ簡缁戝畾token鍒皉eq鐨勬柟娉� bindTokenPara");
                ...
                if (appId > 0) {
                    setTimeout(function() {
                        ...
                        if (aToken.openid && aToken.accessToken) {
                            ...
                            _qc.Console.warn("openid涓巃ccessToken涓㈠け锛岃皟鐢ㄨ姹傚叆鏍� : [" + api + "]锛屾爤澶у皬锛�" + _pendingPool.length);
                        }
                    }, 10);
                } else {
                    _pendingPool.push(req);
                    _qc.Console.warn((_proxy.isReady && appId < 0 ? "token鑾峰彇澶辫触锛岃璋冪敤鐢ㄦ埛鐧诲綍娴佺▼" : "api浠ｇ悊灏氭湭鍒濆鍖栨垚鍔�") + "锛岃皟鐢ㄨ姹傚叆鏍� : [" + api + "]锛屾爤澶у皬锛�" + _pendingPool.length);
                }
